...
A *scene* is a band-exploration preset: a frequency range with a
demodulation setup but no specific channels.  This dialog manages the
band plan (the list of scenes):

  - add, edit (upsert by name), and delete scenes
  - store the current VFO state as a scene
  - apply (load) a scene into the radio
  - import / export the whole band plan as JSON for sharing

The built-in "Free / Full Range" scene (start == end == 0) reproduces
classic free-style VFO tuning.
"""
...
from ui.dialogs.channels_dialog import _bw_label
...
class ScenesDialog(wx.Frame):
    ...
    def _build_ui(self) -> None:
        ...
        imp_btn = wx.Button(panel, label=_("Import…"), name="Import band plan")
        exp_btn = wx.Button(panel, label=_("Export…"), name="Export band plan")
        ...
        # Edit form
        form_box = wx.StaticBox(panel, label=_("Scene"))
        ...
        self._name_ctrl = wx.TextCtrl(panel, name="Scene name")
        _row(_("Name:"), self._name_ctrl)

        self._start_ctrl = wx.TextCtrl(panel, name="Band start")
        _row(_("Band start (MHz, blank = unbounded):"), self._start_ctrl)

        self._end_ctrl = wx.TextCtrl(panel, name="Band end")
        _row(_("Band end (MHz, blank = unbounded):"), self._end_ctrl)

        self._mode_ctrl = wx.Choice(panel, choices=MODES, name="Scene mode")
        self._mode_ctrl.SetSelection(0)
        self._mode_ctrl.Bind(wx.EVT_CHOICE, self._on_mode_change)
        _row(_("Mode:"), self._mode_ctrl)

        self._bw_ctrl = wx.Choice(panel, name="Scene bandwidth")
        ...
        self._default_ctrl = wx.TextCtrl(panel, name="Default frequency")
        ...
        adv_box = wx.StaticBox(panel, label=_("Advanced overrides (optional)"))
        ...
        def _adv_row(label_text: str, ctrl: wx.Window) -> None:
            adv_grid.Add(wx.StaticText(panel, label=label_text), 0, wx.ALIGN_CENTER_VERTICAL)
            adv_grid.Add(ctrl, 1, wx.EXPAND)

        self._nfm_dev_ctrl = wx.TextCtrl(panel, name="NFM deviation override")
        _adv_row(_("NFM deviation (Hz):"), self._nfm_dev_ctrl)

        self._deemph_ctrl = wx.Choice(
            panel, choices=[lbl for _v, lbl in _DEEMPH_CHOICES],
            name="WFM de-emphasis override",
        )
        self._deemph_ctrl.SetSelection(0)
        _adv_row(_("WFM de-emphasis:"), self._deemph_ctrl)

        self._squelch_ctrl = wx.TextCtrl(panel, name="Squelch override")
        ...
        save_btn = wx.Button(panel, label=_("Save Scene"), name="Save scene")
        vfo_btn = wx.Button(panel, label=_("Store Current VFO"), name="Store current VFO as scene")
        ...
        apply_btn = wx.Button(panel, label=_("Apply Selected"), name="Apply selected scene")
        del_btn = wx.Button(panel, label=_("Delete Selected"), name="Delete selected scene")
        ...
        for b in (apply_btn, del_btn, close_btn):
