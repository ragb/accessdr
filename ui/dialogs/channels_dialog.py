...
Manages numbered channel slots grouped into named channel maps:

  - switch / create / delete channel maps
  - add, edit (upsert by number), and delete channels
  - save the current VFO state to a channel slot
  - load a channel into the radio
  - import / export a map as JSON for sharing

Replaces the legacy Bookmarks dialog.
"""
...
def _bw_label(mode: str, value: int) -> str:
...
class ChannelsDialog(wx.Frame):
    ...
    def _build_ui(self) -> None:
        ...
        # --- Map selector row ---
        map_box = wx.StaticBox(panel, label=_("Channel Map"))
        map_sizer = wx.StaticBoxSizer(map_box, wx.HORIZONTAL)
        self._map_choice = wx.Choice(panel, name="Channel map")
        self._map_choice.Bind(wx.EVT_CHOICE, self._on_map_change)
        new_map_btn = wx.Button(panel, label=_("New Map"), name="New map")
        del_map_btn = wx.Button(panel, label=_("Delete Map"), name="Delete map")
        imp_btn = wx.Button(panel, label=_("Import…"), name="Import map")
        exp_btn = wx.Button(panel, label=_("Export…"), name="Export map")
        ...
        # --- Add / edit form ---
        form_box = wx.StaticBox(panel, label=_("Channel"))
        form_sizer = wx.StaticBoxSizer(form_box, wx.VERTICAL)
        grid = wx.FlexGridSizer(cols=2, vgap=6, hgap=8)
        grid.AddGrowableCol(1)

        grid.Add(wx.StaticText(panel, label=_("Number:")), 0, wx.ALIGN_CENTER_VERTICAL)
        self._num_ctrl = wx.SpinCtrl(panel, min=1, max=999, name="Channel number")
        grid.Add(self._num_ctrl, 0)

        grid.Add(wx.StaticText(panel, label=_("Label:")), 0, wx.ALIGN_CENTER_VERTICAL)
        self._label_ctrl = wx.TextCtrl(panel, name="Channel label")
        grid.Add(self._label_ctrl, 1, wx.EXPAND)

        grid.Add(
            wx.StaticText(panel, label=_("Frequency (MHz or kHz):")),
            0, wx.ALIGN_CENTER_VERTICAL,
        )
        self._freq_ctrl = wx.TextCtrl(panel, name="Channel frequency")
        grid.Add(self._freq_ctrl, 1, wx.EXPAND)

        grid.Add(wx.StaticText(panel, label=_("Mode:")), 0, wx.ALIGN_CENTER_VERTICAL)
        self._mode_ctrl = wx.Choice(panel, choices=MODES, name="Channel mode")
        self._mode_ctrl.SetSelection(0)
        self._mode_ctrl.Bind(wx.EVT_CHOICE, self._on_mode_change)
        grid.Add(self._mode_ctrl, 0)

        grid.Add(wx.StaticText(panel, label=_("Bandwidth:")), 0, wx.ALIGN_CENTER_VERTICAL)
        self._bw_ctrl = wx.Choice(panel, name="Channel bandwidth")
        grid.Add(self._bw_ctrl, 0)

        form_sizer.Add(grid, 0, wx.EXPAND | wx.ALL, 6)

        form_btns = wx.BoxSizer(wx.HORIZONTAL)
        save_btn = wx.Button(panel, label=_("Save Channel"), name="Save channel")
        vfo_btn = wx.Button(panel, label=_("Store Current VFO"), name="Store current VFO")
        ...
        load_btn = wx.Button(panel, label=_("Load Selected"), name="Load selected channel")
        del_btn = wx.Button(panel, label=_("Delete Selected"), name="Delete selected channel")
    ...
    def _refresh_list(self) -> None:
        ...
        for ch in cmap.sorted_channels():
            ...
            self._list.SetItem(idx, 4, _bw_label(ch.mode, ch.bandwidth))
